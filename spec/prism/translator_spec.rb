...
RSpec.describe Prism::Translator do
  ...
  it "pushes directly to the current branch with LLM commit message when llm_commit_messages is true" do
    Dir.mktmpdir do |dir|
      source_path = File.join(dir, "locales/en.json")
      translator = build_translator(source_file: source_path, target_languages: ["fr"], delivery_method: "push", llm_commit_messages: true)
  ...
  it "pushes directly to the current branch with default commit message when llm_commit_messages is false" do
    Dir.mktmpdir do |dir|
      source_path = File.join(dir, "locales/en.json")
      translator = build_translator(source_file: source_path, target_languages: ["fr"], delivery_method: "push", llm_commit_messages: false)
      repo = translator.instance_variable_get(:@repo)
      ...
      allow(repo).to receive(:changed_files).and_return(["locales/fr.json"])
      allow(repo).to receive(:relative_path).and_return("locales/fr.json")
      allow(repo).to receive(:commit).with("Update translations").and_return(["ok", instance_double(Process::Status, success?: true)])
      expect(repo).not_to receive(:checkout_new_branch)
      expect(repo).to receive(:push).with("main", remote: "origin").and_return(["ok", instance_double(Process::Status, success?: true)])
      expect(Prism::GitHubClient).not_to receive(:new)
  ...
  it "creates a pull request with LLM commit message when llm_commit_messages is true" do
    Dir.mktmpdir do |dir|
      source_path = File.join(dir, "locales/en.json")
      translator = build_translator(source_file: source_path, target_languages: ["fr"], delivery_method: "pull_request", llm_commit_messages: true)
      repo = translator.instance_variable_get(:@repo)
  ...
  it "creates a pull request with default commit message when llm_commit_messages is false" do
    Dir.mktmpdir do |dir|
      source_path = File.join(dir, "locales/en.json")
      translator = build_translator(source_file: source_path, target_languages: ["fr"], delivery_method: "pull_request", llm_commit_messages: false)
      repo = translator.instance_variable_get(:@repo)
